# pasted from home-manager unstable, will conflict on 22.11
...
in
{
  ...
  options.wayland.windowManager.hyprland = {
    ...
    xwayland = {
      enable = lib.mkEnableOption "XWayland" // {
        default = true;
      };
    ...
    enableNvidiaPatches =
      lib.mkEnableOption
        "patching wlroots for better Nvidia support";
  ...
  config = lib.mkIf cfg.enable {
    ...
    warnings =
      let
        inconsistent =
          (cfg.systemdIntegration || cfg.plugins != [ ])
          && cfg.extraConfig == ""
          && cfg.settings == { }
        ;
    ...
    xdg.configFile."hypr/hyprland.conf" =
      let
        ...
        shouldGenerate =
          cfg.systemdIntegration || cfg.extraConfig != "" || combinedSettings != { };
        ...
        toHyprconf =
          ...
          let
            ...
            '';
            sections = filterAttrs (_: isAttrs) attrs;

            mkFields = generators.toKeyValue { listsAsDuplicateKeys = true; };
            allFields = filterAttrs (_: v: !(isAttrs v)) attrs;
            importantFields =
